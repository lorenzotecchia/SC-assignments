...
int main() {
  ...
  std::cout << "laplace jacobi: " << jres.iterations
            << " iters, residual=" << jres.final_residual << "\n";
  ...
  auto sres =
      sor_solve(N, omega, tolerance, max_iter, mask_empty, apply_boundary);
  std::cout << "laplace sor (baseline): " << sres.iterations
  ...
  auto kres =
      sor_solve(N, omega, tolerance, max_iter, mask_sink, apply_boundary);
  std::cout << "laplace sor (sinks): " << kres.iterations
            << " iters, residual=" << kres.final_residual << "\n";
  ...
  // Omega sweep for K: measure iteration count vs omega with sinks present.
  {
    ...
    for (double w = 1.0; w < 2.0; w += 0.05) {
      auto r_empty =
          sor_solve(N, w, tolerance, max_iter, mask_empty, apply_boundary);
      auto r_sink =
          sor_solve(N, w, tolerance, max_iter, mask_sink, apply_boundary);
  ...
  fill_mask(mask_ins, {{20, 15, 30, 35}}, // one rectangle as insulator
            CellType::INSULATOR);

  auto lres =
      sor_solve(N, omega, tolerance, max_iter, mask_ins, apply_boundary);
  std::cout << "laplace sor (insulator): " << lres.iterations
            << " iters, residual=" << lres.final_residual << "\n";
